...
async function inspectAndRender(wb,sheetName,tag,range='A1:W8') {
  ...
  await fs.writeFile(path.join(args.previewDir||'/tmp/libai-workbooks-ec19c7b46f92',`${tag}.png`),new Uint8Array(await img.arrayBuffer()));
}
...
if(mode==='probe') {
  ...
  await out.save(`/tmp/libai-workbooks-ec19c7b46f92/capacity_probe_${n}.xlsx`);
  report('probe_saved');
}
