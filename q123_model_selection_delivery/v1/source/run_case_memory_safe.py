...
from model import Config,Model,Environment,T0,C0,CL,CD,latent_heat
...
def main(config_path,output):
    ...
    def boundary_rates(tt,yy):
        # Same boundary law, independently evaluated without interior FV work.
        a=yy.reshape(m.n,2);ib=m.bound;T=a[ib,0];C=a[ib,1]
        js,drive,Ci,Ta,W=m.surface(tt,T,C)
        qc=cfg.hT*(Ta-T)
        ql=latent_heat(T)*js if cfg.latent and not cfg.baseline else np.zeros_like(js)
        hs=CL*(T-T0)*js if not cfg.baseline else np.zeros_like(js)
        return np.array([np.dot(area,x) for area in (m.side[ib],m.end[ib]) for x in (js,qc,ql,hs)])
    def record(tt,yy):
    ...
    while t<bound-1e-8:
        ...
        for ta,tb in zip(sol.t[:-1],sol.t[1:]):
            ...
            for gx,gw,cu in [(gx5,gw5,cum),(gx3,gw3,cum3)]:
                ...
                rr=np.array([boundary_rates(tx,ys[:,j]) for j,tx in enumerate(times)])
                cu+=dh*(gw@rr)
        if end<=1800:
            step=1.
