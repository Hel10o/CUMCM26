...
def main(n,ceq,out):
    ...
    # Explicit coloured finite-difference Jacobian avoids cancellation of
    # Celsius T0 + H/S at H=0 in the generic tiny-H finite-difference step.
    jac_groups=[]
    for var in [0,1]:
        for color in range(3):
            cells=np.arange(color,n,3);columns=2*cells+var;rr=[];cc=[]
            for c in cells:
                for dest in range(max(0,c-1),min(n,c+2)):
                    for v in [0,1]:rr.append(2*dest+v);cc.append(2*c+var)
            jac_groups.append((var,columns,np.array(rr),np.array(cc)))
    def jac(t,y):
        rows=[];cols=[];vals=[]
        for var,columns,rr,cc in jac_groups:
            h=1. if var==0 else 1e-6
            yp=y.copy();ym=y.copy();yp[columns]+=h;ym[columns]-=h
            delta=(rhs(t,yp)-rhs(t,ym))/(2*h)
            rows.append(rr);cols.append(cc);vals.append(delta[rr])
        return coo_matrix((np.concatenate(vals),(np.concatenate(rows),np.concatenate(cols))),shape=(2*n,2*n)).tocsc()
    def event(t,y):
    ...
    while t<864000:
        ...
        sol=solve_ivp(rhs,(t,end),y,method='Radau',jac=jac,rtol=1e-8,atol=np.tile([.003,2e-11],n),max_step=20 if not future else 600,events=event,dense_output=False)
