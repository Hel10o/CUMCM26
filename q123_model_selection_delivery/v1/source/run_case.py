...
def main(config_path,output):
    ...
    available_event=(cfg.ceq<.15 or cfg.baseline) and cfg.question!=1
    ...
    result=dict(initial=initial,final=final,t_final=t,event_equal_s=tcrit,execution_s=t if tcrit is not None else None,strict_pass=bool(final['maxC']<.15) if tcrit is not None else False,
                status='crossed_and_continued' if tcrit is not None else ('q1_complete' if cfg.question==1 else ('equilibrium_above_threshold' if cfg.ceq>=.15 and not cfg.baseline else 'not_crossed_in_horizon')),
