"""Replace the Q3 workflow with white boxes without rebuilding numerical files.
...
BASELINE_COMMIT = "6496064c62f5495d0e4d324905bff0cd782cb2db"
BASELINE_ZIP_SHA256 = "eceb65a8936b3a2b09182e3f37f71c7e61e79a96ed60bf9d880c2ba752f3c642"
EVIDENCE = package.BASE / "evidence" / "q3_white_refresh_20260913"
SOURCE_LIST = "support/图表来源清单.md"
FROZEN_WORKFLOW_SHA256 = "4c5e1ea9a2c3a75cd616c4cc6943a5c79a77390caeacdffc14def05fdb7e9428"
...
def current_body_figures():
    """Read body figure references only; unrelated AI-source edits are excluded."""
...
def main():
    assert package.sha256(package.PAPER / "figures/q3_white/q3_workflow.pdf") == FROZEN_WORKFLOW_SHA256
    ...
    current_figures = {
        ...
        for path in current_body_figures()
    }
    assert len(old_figures) == 12 and len(current_figures) == 12
    assert old_figures - current_figures.keys() == {package.TOP + "/figures/q3_intro/q3_workflow.pdf"}
    assert current_figures.keys() - old_figures == {package.TOP + "/figures/q3_white/q3_workflow.pdf"}
    ...
    temporary = output_dir / "A题_支撑材料.q3-white-refresh.tmp"
    ...
    report = {
        "operation": "Q3 white-box workflow replacement and source-list packaging refresh only",
        "figure_reference_scope": "main.tex and sections/*.tex only; unrelated ai_details.tex excluded",
        "ai_pdf_byte_identical_to_prior_zip": updated[package.TOP + "/AI工具使用详情.pdf"] == old[package.TOP + "/AI工具使用详情.pdf"],
        "baseline_commit": BASELINE_COMMIT,
        "baseline_zip_sha256": BASELINE_ZIP_SHA256,
        "baseline_report": "q3_white_refresh_20260913/prior_report.json",
