...
--resume reuses only checkpoints belonging to the same input hash and model.
"""
...
def main():
    ...
    if out==ROOT/'inputs' or any(q in out.parts for q in ('q1_delivery','q1_complete_delivery')):raise ValueError('Protected input/Q1 output location')
    inputs={str(q.name):file_hashes(q) for q in (a.input,a.template)}
    out.mkdir(parents=True,exist_ok=True);v=out/'validation';v.mkdir(exist_ok=True);log=out/'run_state.json'
    if log.exists():
        state=json.loads(log.read_text())
        if not a.resume:raise FileExistsError('Output has run_state.json; use a new directory or explicit --resume')
        if state['input_hashes']!=inputs:raise ValueError('Input hash changed; refusing cached results')
    else:state={'input_hashes':inputs,'stages':{},'complete':False,'model_version':'q2-effective-local-v1','excel_engine':a.excel_engine}
    ...
    def done_npz(name):return a.resume and (v/(name+'.npz')).exists() and (v/(name+'.json')).exists()
    selected=STAGES if a.stage=='all' else [a.stage]
    for stage in selected:
        if a.resume and state['stages'].get(stage,{}).get('passed'):continue
        ...
        try:
            if stage=='mesh':
                ...
                for nr,nz in ((20,40),(20,80),(40,80)):
                    ...
                    if not done_npz(name):save_solution(v/(name+'.npz'),solve_axisymmetric(env,nr,nz,verbose=True,resume_path=v/'checkpoints'/(name+'.npz')))
    ...
    state['complete']=all(state['stages'].get(s,{}).get('passed',False) for s in STAGES)
    if state['complete']:
