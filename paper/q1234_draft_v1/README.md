...
最新一轮仅增加原流程图行间距：图高6.4→8.0 cm，图宽、33条文字、字号和箭头关系不变。正文与图注不改，仍为157页；除第2页外，其余156页像素一致。见[行间距修订记录](evidence/workflow_spacing_20260912/流程图行间距修订记录.md)与[重绘入口](workflow_spacing/README.md)。

本轮依据用户的[图片方案原件](evidence/figure_refinement_20260912/用户图表方案_原件.md)，从实际读取的 `e1ae2bc23917cbbbd357cb14ca795896f4a39f25` 六图版本比较取舍。四张既有图细修，降维及环境两张图保留；新增一张全模型流程图，现行共七图。第25页长目录改为简短来源说明，具体文件与哈希列入[图表来源清单](support/图表来源清单.md)。正文科学语句、数值结果、公式、规定表、摘要及结论不改，字号、行距、页边距与标题格式不变；局部图注/读图导览及图文浮动调整见[本轮完整记录](evidence/figure_refinement_20260912/现有图细修与流程图修订记录.md)与[逐图选择](evidence/figure_refinement_20260912/existing_figure_decisions.md)。

当前PDF为 **157页：摘要1页、正文29页（物理页2–30）、附录127页**。最终37项PDF机器检查通过，六张题设表279个显示单元格一致；摘要与127页附录（含页脚）和基线像素完全相同。局部调整仅影响图形、图注及相关图文摆放，末次编译后的独立核验与目视记录见本轮证据目录。源稿ZIP与外置清单随本轮更新。
