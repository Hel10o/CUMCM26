...
图表已随源稿提供，编译不需要重新求解。现行六图由 `build_visual_assets.py` 调用 `visual_revision/` 三组脚本，只读冻结数组并写到 `figures/visual_revision/`；详见[新图复现说明](visual_revision/README.md)。原 `build_assets.py` 和 `build_dimension_reduction.py` 保留为历史入口，不用于覆盖本轮新图。重绘需要完整仓库中的原数组及科学Python依赖，独立源稿ZIP直接使用已生成图编译。原[资产清单](evidence/assets_manifest.json)保留，本轮来源和哈希单独记录。
