...
[图表来源清单](图表来源清单.md)对应现行11幅正文图：四张分问流程图、一张问题一中截面与边界示意图、六张数值图。第六章的流程图与热图按用户方案修订，并新增边界图，来源见[本轮记录](../evidence/ch6_revision_20260913/figure_sources.json)。清单保留降维配对数组的完整路径和哈希；旧图仅作历史保留，原支撑源码与数值内核不变。
