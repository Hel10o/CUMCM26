...
def main():
    ...
    for col,(key,cmap,title,unit,lo,hi,ticks) in enumerate([
        ...
        left=.095+col*.49
        ax=fig.add_axes([left,.23,.365,.53])
        ...
        if col==0:
            ...
            fig.text(left,.070,f'末时轴心 / 表面：{values[0]:.4f} / {values[1]:.4f} °C',fontsize=7.1,color=INK)
            fig.text(left,.025,f'径向温差 ΔT = {delta:.4f} K',fontsize=7.1,color=INK)
        else:
            assert f'{values[0]:.7f}'=='2.5499924'
            fig.text(left,.070,f'末时轴心 ≈ {values[0]:.7f} kg/kg',fontsize=7.1,color=INK)
            fig.text(left,.025,f'末时表面：{values[1]:.4f} kg/kg',fontsize=7.1,color=INK)
    ...
    # Direct time labels free the lower area for a literal endpoint-difference marker.
    for h,color in zip([.5,1.5,3],palette):
        i=at(q2['time_s'],h*3600)
        right.text(.43,field[i,0,1]+.035,f'{h:g} h',color=color,fontsize=7.1)
    c_axis,c_surface=terminal[0,1],terminal[-1,1]
    right.plot([0,.16],[c_axis,c_axis],color=GREY,ls=':',lw=.65)
    right.plot([.16,2],[c_surface,c_surface],color=GREY,ls=':',lw=.65)
    right.annotate('',xy=(.16,c_axis),xytext=(.16,c_surface),arrowprops={'arrowstyle':'<->','lw':.8,'color':GREY})
    right.text(.23,(c_axis+c_surface)/2,'ΔC',fontsize=7.5,color=INK,va='center')
    ...
    ev.finish(__file__,{'q1':q1_records,
        'q1_display':'All saved second/radial samples; unchanged heatmap with endpoint deltaT and seven-decimal axis moisture, no front threshold.',
        ...
        'q2_terminal_deltas':{'temperature_K_display':'0.1169','C_display':'0.7581','C_arrow':'literal stored 3h center-surface difference'},
