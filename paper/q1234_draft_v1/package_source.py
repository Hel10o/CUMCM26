...
roots = ["main.tex", "ai_details.tex", "body_preview.tex", "build.ps1",
         "build_assets.py", "build_dimension_reduction.py", "build_visual_assets.py", "build_refined_assets.py", "figures/dimension_reduction.png", "validate_paper.py", "package_source.py", "README.md"]
...
for path in (BASE / "evidence").rglob("*"):
    if path.is_file() and path.suffix in {".md", ".json", ".txt", ".pdf", ".html"} and path.name != "source_package.json":
        files.append(path)
for directory in ["sections", "tables", "figures", "support", "visual_revision", "visual_refinement"]:
