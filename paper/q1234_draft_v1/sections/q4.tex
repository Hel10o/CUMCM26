...
问题四使用附录4物性，并根据附件2计入尺寸变化\cite{problem}。
...
同时
$\Ds f/\dd t=\partial_t f|_\xi+(v_{s,r}-w_r)\partial_r f
=\partial_t f|_\xi$，
从而一维参考域方程为
\begin{equation}
 \begin{aligned}
...
令$x=\xi^2$，径向算子化为
$(4/R^2)\partial_x[x(\cdot)]$，消去轴线显式奇点。
采用120阶Chebyshev配点与Radau时间推进，
在环境及半径输入节点处分段积分，表面温湿由非线性边界联立确定。
水分通量用$F_4'(C)=\exp(-0.30/C)$的原函数梯度表示，
并检查其插值多项式的全部内部实驻点及两端点，以确定全径向极值。
...
与固定几何时不同，必须先将材料坐标转换回实际$r$，
再判断该位置是否仍在药材内；当前表面另列，不能当作固定2\unit{cm}位置。
...
\subsection{有限圆柱全域的数值检验}

二维模型在式\eqref{eq:q4-reference-pde}中加入轴向通量散度，
端面使用相同交换系数，长度仍为$L_0$。
在80个径向区间、128个轴向区间的配对计算中，
二维事件根为183934.287166\unit{s}，
同径向网格的一维根为183934.321385\unit{s}，
相差约$-0.034218\unit{s}$。
这说明端部交换使该配对事件略提前；
两个粗网格绝对根共同含有约3.1\unit{s}的径向偏差，
不能把它全算成几何影响。

不同空间与时间设置下，配对提前量约为0.0286--0.0343\unit{s}。
精细一维根加该差值得到二维临界根估计
183931.177445--183931.183083\unit{s}，
但这不是严格上下界。
直接将已有80$\times$128二维场推进至183931.56\unit{s}时，
其全节点最大含水率为0.150001406623\unit{kg/kg}，仍略高于阈值；
该局部续算继承原粗网格前史，不能消除上述共同径向误差。
因此，本文保留51.0921\unit{h}作为一维条件执行值，
配对估计提供其二维保守方向的支持；
进一步作出充分的有限圆柱全域达标结论，
仍需精细二维正式点及误差与正裕量对照。
这一证据范围不等于用粗网格结果推翻精细主解，
也不等于已经取得连续二维的数学保证。
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=0.92\textwidth]{figures/q4_shrinkage.pdf}
 \caption{观测半径历程（左）及附录4下不同时间的径向含水率剖面（右）。
 半径图标出正式执行时刻；剖面采用当前实际半径坐标，
 各曲线只画至对应时刻的材料表面。}
 \label{fig:q4-shrinkage}
\end{figure}
\FloatBarrier

图\ref{fig:q4-shrinkage}给出半径输入与相应的径向含水率变化。
为分离施加收缩历程的作用，另保持附录4和其余条件不变，
将半径固定为2\unit{cm}作受控对照，
临界时间为129.848360839\unit{h}；
施加观测收缩后为51.092003254\unit{h}，
缩短78.7564\unit{h}，约60.6526\%。
...
所以60.6526\%是同一有效方程内的条件时长差，
不等于固定真实气膜条件下仅由缩短路径得到的效应，更不是实际节能率。

半径重建方式也影响终时。
将线性插值改为保形PCHIP，临界时间为$51.088501642\unit{h}$，
比主线提前约12.6058\unit{s}，明显大于单一插值方案内部的数值预算。
这属于输入重建敏感性，不能当作半径测量误差的置信区间。
此外，$R(t)$本身是施加的观测，复现该曲线不能作为温湿预测的独立验证；
更换环境或交换系数而仍使用原$R(t)$，只能解释为该几何历程下的条件响应。
