...
本轮将问题二图5替换为参考Word图7-2重绘的温度/含水率时空双热图，同步调整图注和一个读图段。仅用已保存数组作图，保留原节点和数值；7.1—7.3、7.5机制图及其他内容、排版指令保持。PDF仍158页（摘要1页、正文30页、附录127页），只有物理第13、14页变化，其余156页像素一致；论文专项25项通过，六表279格一致。已刷新源稿与支撑包，见[图5替换记录](paper/q1234_draft_v1/evidence/q2_heatmap_20260913/图5时空双热图替换记录.md)和[当前双热图及数据来源](paper/q1234_draft_v1/figures/q2_heatmap/README.md)。
